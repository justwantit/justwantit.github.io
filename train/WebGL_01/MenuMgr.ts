class MenuMgr
{
    curMenu: lighttool.htmlui.panel;
    ...
    onInit(gl: WebGLRenderingContext, canv: HTMLCanvasElement)
    {
        this.gl = gl;
        this.canv = canv;
    ...
    creatCylinder(n: number)
    {
        var img = new Image();
        img.onload = () =>
        {
            this.cylinder = new Cylinder(this.gl, this.canv, n, 0.4, 1);
            this.cylinder.initdata();//初始化模型数据
            this.cylinder.initImg(img);//初始化贴图信息
            this.cylinder.drawself();
        }
